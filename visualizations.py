...
def visualize_attention(filename, output_dir, attentions, pred, pad_width,
    ...
    if flag is None:
        filestring = 'predict-{}'.format(1)
        idx = 2
        while filestring in os.listdir(output_dir):
            filestring = 'predict-{}-{}'.format(2, idx)
            idx += 1
        out_dir = output_dir
    elif flag:
    ...
    with open(os.path.join(out_dir, 'word.txt'), 'w') as fword:
        ...
        # img_out_frames[10].save('a.jpg')
        for i in range(10):
            img_out_frames[i].save('a'+str(i)+'.jpg')
            break
        # Save cropped and animated images.
        output_animation = os.path.join(out_dir, 'image.jpeg')

        img_out_frames[0].save(
            output_animation, format='jpeg', append_images=img_out_frames[1:10])
        # , save_all=True, loop=999,duration=500,
...
def map_attentions(img_data, attentions, pred, pad_width, pad_height,
    ...
    #img_out_i = np.zeros(img_data.shape)
    img_out_frames = []
