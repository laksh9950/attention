...
class Model(object):
    def __init__(self,
        ...
        max_resized_width = 1. * max_image_width / \
            max_image_height * DataGen.IMAGE_HEIGHT
        ...
        with tf.device(device_id):
            ...
            self.height_float = tf.constant(
                DataGen.IMAGE_HEIGHT, dtype=tf.float64)

            self.img_pl = tf.placeholder(
                tf.string, name='input_image_as_bytes')
            ...
            self.perm_conv_output = tf.transpose(
                self.conv_output, perm=[1, 0, 2])
            ...
            with tf.control_dependencies([insert]):
                ...
                trans_output = tf.map_fn(
                    lambda m: tf.foldr(
                        lambda a, x: tf.cond(
                            ...
                            lambda: '',
                            lambda: table.lookup(
                                x) + a  # pylint: disable=undefined-variable
                        ),
                        m,
                        initializer=''
                    ),
                ...
                trans_outprb = tf.gather(
                    trans_outprb, tf.range(tf.size(trans_output)))
            ...
            if not self.forward_only:  # train
                ...
                if self.reg_val > 0:
                    ...
                    logging.info(
                        'Adding %s regularization losses', len(reg_losses))
                    logging.debug('REGULARIZATION_LOSSES: %s', reg_losses)
                    loss_op = self.reg_val * \
                        tf.reduce_sum(reg_losses) + loss_op
    ...
    def predict(self, image_file_data):
        ...
        output_feed = [self.prediction, self.probability,
                       self.attention_decoder_model.attentions]
        ...
        if isinstance(image_file_data, str):
            img_file = open(image_file_data, 'rb')
            img = Image.open(img_file)
        else:
            img = Image.open(BytesIO(image_file_data))
        ...
        pad_height = DataGen.IMAGE_HEIGHT
        pad_width = self.max_width
        ...
        coordinates = []
        for idx in [0, 3, 4, 5, 8, 9, 10, 13]:
            attention = attns[0][idx]
        ...
        if normalize:
            ...
            attention_14[attention_14 >= score_14 * threshold] = 1
            attention_14[attention_14 < score_14] = 0

        # Resize attention to the image size, cropping padded regions.
        attention_1 = Image.fromarray(attention_1)
        attention_4 = Image.fromarray(attention_4)
        attention_10 = Image.fromarray(attention_10)
        attention_14 = Image.fromarray(attention_14)
        ...
        attention_10 = attention_10.resize(
            (int(pad_width*width_resize_ratio),
             int(pad_height*height_resize_ratio)),
            Image.NEAREST)
        attention_14 = attention_14.resize(
            (int(pad_width*width_resize_ratio),
             int(pad_height*height_resize_ratio)),
            Image.NEAREST)
        ...
        return (text, probability, coordinate_1, coordinate_4, coordinate_10, coordinate_14)
        '''
        ...
        for batch in s_gen.gen(1):
            ...
            if incorrect:
                correctness = step_accuracy + \
                    " ({} vs {}) {}".format(output, ground, comment)
            else:
            ...
            logging.info('Step {:.0f} ({:.3f}s). '
                         ...
                         'loss: {:f}, perplexity: {:0<7.6}, probability: {:6.2%} {}'.format(
                             ...
                             result['loss'],
                             math.exp(result['loss']) if result['loss'] < 300 else float(
                                 'inf'),
                             probability,
                             correctness))
    ...
    def train(self, num_epoch=10):
        ...
        self.saver_all.save(self.sess, self.checkpoint_path,
                            global_step=self.global_step)
